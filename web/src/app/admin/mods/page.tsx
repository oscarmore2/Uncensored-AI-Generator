    ...
    <div>
      ...
      <form onSubmit={grantMod} className="glass rounded-3xl p-5 mb-6 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[200px]">
          <label className="text-xs text-ink-muted block mb-1">将已有用户提升为审核员</label>
          <input
            value={username}
            onChange={(e) => setUsername(e.target.value)}
            placeholder="用户名"
            className="w-full bg-surface border border-line rounded-2xl px-4 py-2.5 text-sm outline-none"
          />
        </div>
        <button
          type="submit"
        ...
        <table className="w-full text-sm min-w-[700px]">
          ...
          <tbody>
            {mods.map((m) => (
              <tr
                key={m.id}
                className={`border-b border-line hover:bg-black/[0.03] ${m.enabled ? "" : "opacity-60"}`}
              >
                <td className="px-4 py-3 font-mono text-ink-muted">#{m.id}</td>
                <td className="px-4 py-3 font-medium">{m.username}</td>
                <td className="px-4 py-3">
                  {m.enabled ? (
                    <span className="text-xs px-2 py-0.5 bg-emerald-500/15 text-emerald-700 rounded-full">已启用</span>
                  ) : (
                    <span className="text-xs px-2 py-0.5 bg-red-500/15 text-red-700 rounded-full">已停用</span>
                  )}
                </td>
                ...
                <td className="px-4 py-3 text-right whitespace-nowrap">
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1 bg-black/[0.03] hover:bg-black/[0.06] border border-line rounded-lg disabled:opacity-50"
                  >
                    撤销角色
                  </button>
                </td>
