...
> 状态：**S0 / S1 已实现，S2 进行中**（见 9.1–9.3），待确认项已全部收敛为第十节的决定。
...
选区那五个动作**已经从写死的 switch 变成库里的行**。管理端能改提示词、改名称图标、
限定模式、停用、恢复默认。剩下的是 `manual` 时机与魔法指令归位，见本节末尾。
...
- **`manual` 时机**：前端还没有「工具栏按钮 → 跑一个整段技能」这条路径。
- **魔法指令归位**：它现在仍是独立链路（自己的路由、JSON 输出解析、反向提示词、
  本地兜底、以及旧的 `ai_credits_per_1k_tokens` 费率）。归位要把这些一并搬过来，
  是独立的一块，与选区那半分开做。
