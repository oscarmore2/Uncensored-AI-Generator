...
| 子系统 | 覆盖需求 | 独立性 |
| --- | --- | --- |
| A. 提示词编辑器（放大弹窗 + Markdown） | 1, 2 | 完全独立，可先做 |
...
我不知道 Wan 系列、H3 的引用语法具体是什么。**我不会去猜**——
猜错的后果是提示词里出现模型不认识的字符串，它当普通文字读进去，
生成结果悄悄跑偏，而且很难归因。

我能做的是把机制和管理端建好，语法表由你填（或者你把各家文档给我，我来填）。
...
「支持 Markdown」有两种读法，做出来完全不同：

1. **仅编辑辅助**：编辑器提供分段、列表、预览，但**上行的仍是原始文本**
2. **参与提交**：把 Markdown 渲染后的内容发给模型

我按 **(1)** 设计。因为提示词是原样送进模型的，任何渲染都会改变模型实际读到的
字符串——`**强调**` 会变成 `<strong>强调</strong>` 或丢掉星号，两种都不是
用户写下时的意图。若你要的是 (2)，告诉我，这是另一套做法。
...
1. **Markdown 的语义**：仅编辑辅助（我的默认），还是要参与提交？
2. **各家引用语法**：Wan / H3 等的具体格式。给我文档或示例我来填；
   在此之前 B2 卡住，但 B1 和其他阶段都能推进。

确认前我可以直接开 **C1**——它不依赖上面任何一条。
