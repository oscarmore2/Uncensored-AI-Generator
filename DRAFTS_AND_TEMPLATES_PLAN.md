...
| | 自动保存（VIP） | 手动（所有人） |
| --- | --- | --- |
| 编辑期间 | 每分钟落一次盘，离开页面补一次 | **一个字都不往服务端写** |
| 本地 IndexedDB | 照旧兜刷新 | 照旧兜刷新 |
| 挂载时 | 直接从数据库恢复（换设备也能接着写） | 不动界面，等用户点「恢复上次编辑」 |
| 另存为 | 有 | 无（VIP 功能） |

**生成一旦开始就完全停写。** 那一刻的内容已经交给上游，再写草稿会让
「这条草稿对应哪次生成」说不清。生成失败回到可编辑状态，保存自然回来——
所以「未开始生成」与「上次生成失败」的草稿都能保存并继续生成。

门禁在服务端，不是靠前端藏按钮：`/api/me/draft-autosave` 开启需 VIP，
`POST /api/drafts` 带 `save_as` 需 VIP。**关闭自动保存任何人都可以**，
否则 VIP 过期的用户会被永久卡在开启状态上。
...
| 阶段 | 内容 | 完成标准 |
| --- | --- | --- |
| **C1** | Draft 表 + 快照编解码 + 活动草稿自动回写 | ✅ 已完成 |
| **C2** | 「我的草稿 / 我的作品」分栏 + 点击草稿跳回对应模式 | ✅ 已完成 |
| **C3** | 生成后删除 + 打开草稿页对账 | ✅ 已完成 |
| **D1** | PromptTemplate 表 + 从草稿/作品存为模板 | ✅ 已完成 |
| **D2** | 模式内列出模板 + 套用确认弹窗 | ✅ 已完成 |
| **D3** | 兼容性三档判定与标记 | ✅ 已完成 |
| **A1** | 放大弹窗 + 素材轨 + 引用插入 | ✅ 已完成 |
| **A2** | Markdown 编辑辅助 | ✅ 已完成 |
| **B1** | ModelRefSyntax 表 + 渲染器 + 管理端 | ✅ 已完成 |
| **B2** | 填各家语法 | 管理端已备一键填入；上线后由你在 /admin/ref-syntax 添加 |
